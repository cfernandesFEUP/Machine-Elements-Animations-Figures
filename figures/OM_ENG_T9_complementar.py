...
vf2 = np.add(dm.matrix(dm.inv(alpha),vi2), (O2*np.ones((evsize+rasize,2))).T)
vf2t = np.add(dm.matrix(phib+dm.inv(alpha),vi2), (O2*np.ones((evsize+rasize,2))).T)
...
ax.plot(xh[:end], yh[:end], zh[zh<=b], zdir=zv, color='r',label='hélice primitiva')

# ax.plot([0,0], [0,0], [-0.1*b,1.1*b], zdir=zv,linestyle='-.',color='k',lw=0.5)
...
ax.plot([0,0], [.5*r1,.5*r1], [0,b], zdir=zv,linestyle='-',color='k',lw=0.5)

# ax.plot([0,-b*np.tan(beta)], [.5*r1,.5*r1], [0,b], zdir=zv,linestyle='-',color='k',lw=0.5)
# ax.plot([xh[0],xh[-1]], [yh[0],yh[-1]], [0,pz], zdir=zv,linestyle='-',color='k',lw=0.5)
# Cylinder

Xc,Yc,Zc = data_for_cylinder_along_z(0.,0.,.5*r1,0,b)
ax.plot_surface(Xc, Yc, Zc,color='k', alpha=0.25)

angle = np.linspace(np.pi/2, np.pi/2+phib,100)
x = .5*r1*np.cos(angle)
y = .5*r1*np.sin(angle)
ax.plot(x,y,zs=b,color='b')
...
# plt.show()
plt.savefig('pdf/complementar.pdf', bbox_inches = 'tight',

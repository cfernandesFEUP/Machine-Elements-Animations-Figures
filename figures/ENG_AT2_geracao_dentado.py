...
vl = np.array([np.array([xRack[0],xRack[-1]]), np.array([2*m, 2*m]), np.zeros(2), np.ones(2)])
vup = np.zeros((len(vl),2,len(phi)))

from matplotlib.backends.backend_pdf import PdfPages
pp = PdfPages('pdf/anim/geracao.pdf')
...
def animate(i):
    ...
    ax.fill(np.append(xp, vup[0,::-1,i]), np.append(yp, vup[1,::-1,i]), '#fafafa')
    pp.savefig(fig, bbox_inches = 'tight', transparent=True, dpi=10)
    return line, linep,
...
ani = anm.FuncAnimation(fig, animate, init_func=init,
                               frames=len(phi), blit=False, repeat=False, interval=10)
